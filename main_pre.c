...
xcb_gcontext_t color_gc(xcb_connection_t *c, xcb_window_t w, xcb_screen_t *s,
 ...
 xcb_alloc_color_reply_t *rep = xcb_alloc_color_reply(c,
  xcb_alloc_color(c, s->default_colormap, f2i16(r), f2i16(g), f2i16(b)),
  ((void *)0));
 ...
 *sr = bit_set(*sr, 1, a == 0);
 *sr = bit_set(*sr, 7, a >> 7 & 1);
 return a;
}
# 108 "main.c"
void op_ADC(uint8_t (*get)(uint16_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t), void (*set)(uint8_t, uint16_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t), uint16_t *pc, uint8_t *ac, uint8_t *x, uint8_t *y, uint8_t *sr, uint8_t *sp) {
 uint16_t r = *ac + (*get)(pc, ac, x, y, sr, sp) + (*sr >> 0);
 uint8_t r8 = r;
 *sr = bit_set(*sr, 0, r >> 8 & 1);
 int o = !((*ac >> 7 & 1) ^ ((*get)(pc, ac, x, y, sr, sp) >> 7 & 1)) &&
  ((*ac >> 7 & 1) ^ (r >> 7 & 1));
 *sr = bit_set(*sr, 6, o);
 sr_nz(sr, r8);
 *ac = r8;
}
void op_LDA(uint8_t (*get)(uint16_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t), void (*set)(uint8_t, uint16_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t), uint16_t *pc, uint8_t *ac, uint8_t *x, uint8_t *y, uint8_t *sr, uint8_t *sp) { *ac = sr_nz(sr, (*get)(pc, ac, x, y, sr, sp)) }
void op_STA(uint8_t (*get)(uint16_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t), void (*set)(uint8_t, uint16_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t), uint16_t *pc, uint8_t *ac, uint8_t *x, uint8_t *y, uint8_t *sr, uint8_t *sp) { (*set)(*ac, pc, ac, x, y, sr, sp) }


struct addr { uint8_t (*get)(uint16_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t) get; void (*set)(uint8_t, uint16_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t) set; };




uint8_t addr_get_imm(uint16_t *pc, uint8_t *ac, uint8_t *x, uint8_t *y, uint8_t *sr, uint8_t *sp) { return 0; } void addr_set_imm(uint8_t, uint16_t *pc, uint8_t *ac, uint8_t *x, uint8_t *y, uint8_t *sr, uint8_t *sp) { return 0; } struct addr addr_imm = { .get = addr_get_imm, .set = addr_set_imm };

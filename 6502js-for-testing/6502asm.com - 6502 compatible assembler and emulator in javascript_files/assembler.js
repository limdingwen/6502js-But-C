...
function gotoAddr() {
	var inp = prompt( "Enter diao address or label", "" );
	var addr = 0;
	if( findLabel( inp ) ) {
		addr = getLabelPC( inp );
	} else {
...
function print_difflog(pc, name, old, n) {
	difflog_buffer +=
		difflog_count + ": Ins " + memory[pc].toString(16).padStart(2, "0")
		+ " @ " + pc.toString(16).padStart(4, "0") + ", " + name + " " +
		old.toString(16).padStart(2, "0") +
...
var backupPC = regPC;
  memory[0xfe]=Math.floor( Math.random()*256 ); // RANDOM
  memory[0xfe]=7; // DECIDED BY COIN FLIP (for difflog)
  inst[popByte()]();
  ...
  // Difflog
	for (var i = 0; i < memory.length; i++) {
		...
		difflog_buffer +=
			...
			+ " @ " + backupPC.toString(16).padStart(4, "0") + ", Memory " +
			i.toString(16).padStart(4, "0") + ", " +
			difflog_prev_mem[i].toString(16).padStart(2, "0") +
			" -> " + memory[i].toString(16).padStart(2, "0") + "\n";
		difflog_prev_mem[i] = memory[i];
	}

	if (difflog_prev_a != regA)
		print_difflog(backupPC, "AC", difflog_prev_a, regA);
	if (difflog_prev_x != regX)
		print_difflog(backupPC, "X", difflog_prev_x, regX);
	if (difflog_prev_y != regY)
		print_difflog(backupPC, "Y", difflog_prev_y, regY);
	if (difflog_prev_p != regP)
		print_difflog(backupPC, "SR", difflog_prev_p, regP);
